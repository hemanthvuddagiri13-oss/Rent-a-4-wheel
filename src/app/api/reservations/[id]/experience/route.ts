...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  } catch (error) { return Response.json({ error: error instanceof MarketplaceError ? error.message : "Unable to complete this action. Refresh and try again." }, { status: error instanceof MarketplaceError ? error.status : 409 }); }
}
