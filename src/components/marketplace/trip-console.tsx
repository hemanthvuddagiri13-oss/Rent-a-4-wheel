...
export function TripConsole({ reservationId }: { reservationId: string }) {
  ...
  return <div className="space-y-6">
    ...
    {(pretrip||returning)&&<Panel title={data.role==="HOST"?"Host checklist":"Your checklist"}><ol className="grid gap-3 sm:grid-cols-2">{(returning?["Record return location, mileage and fuel","Photograph exterior and interior","Review and accept your saved report",data.role==="HOST"?"Compare both reports and complete authorized review":"Coordinate key return with your host"]:[data.role==="HOST"?"Compare identity evidence and the physical license":"Meet your host for identity handoff","Inspect and photograph the vehicle","Review and accept your saved report",data.role==="HOST"?"Release keys only when the gate permits":"Start only after the host releases keys"]).map((text,index)=><li key={text} className="rounded-lg border border-white/10 p-3"><span className="mr-2 font-semibold text-gold">{index+1}.</span>{text}</li>)}</ol><p className="mt-4 text-sm text-muted">Your saved progress is shown below. A locally selected photo is not saved until the inspection submission succeeds. If the connection fails, check your saved report before resubmitting.</p><p className="mt-2 text-sm text-silver">Current pickup/return location: {data.pickupLocation}</p></Panel>}
    <Panel title={data.status === "ACTIVE" ? "Your trip is underway" : returning ? "Return your vehicle" : "Pickup and trip readiness"}>
      ...
      {data.status === "COMPLETED" && <p className="text-silver">Return complete. Check payment status for any required security deposit release. Your bank may take additional time to make funds available.</p>}
    ...
    <Panel title="Identity documents"><p className="mb-4 text-sm text-muted">Documents are private downloads available only through your current access. Files awaiting a security scan cannot be opened. Open clean downloads on your own device for the handoff; do not share them.</p><div className="space-y-3">{data.documents.length ? data.documents.map(d => <p key={d.id} className="text-sm text-silver">{d.type.replaceAll("_", " ")} · {d.status.replaceAll("_", " ")} · {d.malwareScanStatus} {(d.malwareScanStatus === "CLEAN") && <a href={`/api/documents/${d.id}`} target="_blank" rel="noreferrer" className="ml-2 text-gold-bright underline">Download private document</a>}</p>) : <p className="text-silver">No identity documents attached.</p>}</div></Panel>
    ...
    {(pretrip || returning) && !ownReport && <Panel title={returning ? "Your return inspection" : "Your pre-trip inspection"}><form onSubmit={report} className="space-y-5"><p role="status" className="text-sm text-silver">Required photos selected: {["EXTERIOR","INTERIOR"].filter(category=>selectedPhotos[phase+category]).length} of 2. Review both images before submitting.</p><input type="hidden" name="phase" value={phase} /><div className="grid gap-4 sm:grid-cols-2"><label className="grid gap-2 text-sm text-silver">Odometer (miles)<input required type="number" name="mileage" min={data.trip?.startMileage ?? 0} className="workspace-input" /></label><label className="grid gap-2 text-sm text-silver">Fuel / battery (%)<input required type="number" name="fuelLevel" min={0} max={100} className="workspace-input" /></label>{["EXTERIOR", "INTERIOR"].map(category => <ReportPhotoField key={phase+category} onSelected={selected=>setSelectedPhotos(prev=>({...prev,[phase+category]:selected}))} category={category} label={category === "EXTERIOR" ? "Exterior photo" : "Interior photo"} />)}</div><label className="grid gap-2 text-sm text-silver">Damage or condition notes<textarea name="damageNotes" maxLength={2000} className="workspace-input" /><span className="text-xs">Leave blank if there is no damage to report.</span></label><button className={button} disabled={busy}>Submit inspection report</button></form></Panel>}
    <Panel title="Inspection evidence"><p className="mb-4 text-sm text-muted">Open private photo downloads to compare before and after condition. Access is checked each time. Files are not placed in public galleries.</p>{data.reports.length ? data.reports.map(r => <div key={r.id} className="mb-5 rounded-xl border border-white/10 p-4"><h3 className="text-white">{r.role} · {r.phase.replaceAll("_", " ")}</h3><p className="mt-1 text-sm text-muted">Submitted {new Date(r.createdAt).toLocaleString("en-US",{timeZone:data.bookingTimezone})} ({data.bookingTimezone}){r.own?" · By you":" · By the other trip participant"}</p><p className="mt-2 text-sm text-silver">{r.mileage.toLocaleString()} miles · {r.fuelLevel}% fuel / charge</p>{r.damageNotes && <p className="mt-2 text-sm text-silver">{r.damageNotes}</p>}<div className="my-4 flex flex-wrap gap-4">{r.photos.map(p => <a className="inline-flex min-h-11 items-center text-sm text-gold-bright underline" href={`${base}/photos/${p.id}`} key={p.id}>Download {p.category.toLowerCase()} photo</a>)}</div>{r.acceptedAt ? <p className="text-sm text-gold-bright">Accepted by author</p> : r.own && <button disabled={busy} className={button} onClick={() => send(`/condition-reports/${r.id}/accept`, {})}>I accept this report as accurate</button>}</div>) : <p className="text-silver">Inspection reports will appear here after submission.</p>}</Panel>
    ...
    <Panel title="Need help?"><p className="text-silver">For immediate danger or a medical emergency, call 911. For trip or roadside coordination, contact your host or support. Do not release keys or start driving until the checklist permits it.</p><a className="mt-3 inline-flex min-h-11 items-center underline" href={`/connect?view=support&reservationId=${reservationId}`}>Get trip support</a></Panel><Panel title="Trip activity"><ol className="space-y-3 text-sm text-silver">{data.events.map(e => <li key={e.id}>{e.type.replaceAll("_", " ")} · {new Date(e.createdAt).toLocaleString()}</li>)}</ol></Panel>
  </div>;
}
