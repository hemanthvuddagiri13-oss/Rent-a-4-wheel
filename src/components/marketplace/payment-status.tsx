...
type Status = { outcome: string; paidCents: number; refundedCents: number; pendingRefundCents: number; depositStatus: string | null; depositValid: boolean; status: string };
export function PaymentStatus({ id }: { id: string }) {
  ...
  useEffect(() => {
    let live = true;
    async function poll() { try { const r = await fetch(`/api/reservations/${id}/status`, { cache: "no-store" }); if (!r.ok) throw new Error("Payment status unavailable. Refresh to retry."); const d = await r.json(); if (live) setData(d); } catch (e) { if (live) setError(String(e)); } }
  ...
  return <Panel title="Payment, refund and deposit"><p role="alert" className="text-sm text-red-300">{error}</p>{data ? <><p className="mb-4 text-gold-bright" role="status">{data.outcome.replaceAll("_", " ")}</p><dl className="grid grid-cols-2 gap-3 text-sm text-silver"><dt>Rental collected</dt><dd>{formatCurrency(data.paidCents)}</dd><dt>Refund completed</dt><dd>{formatCurrency(data.refundedCents)}</dd><dt>Refund pending</dt><dd>{formatCurrency(data.pendingRefundCents)}</dd><dt>Deposit</dt><dd>{data.depositStatus?.replaceAll("_", " ") || "Not required / not authorized"}</dd></dl>{["payment_failed", "deposit_action_required"].includes(data.outcome) && <button disabled={busy} onClick={retry} className="mt-5 rounded-lg bg-gold px-5 py-3 font-semibold text-black disabled:opacity-50">{busy ? "Checking deposit…" : "Recover deposit authorization"}</button>}<a className="mt-5 block text-sm text-gold-bright underline" href={`/api/reservations/${id}/receipt`}>Download payment receipt</a></> : <p className="text-silver">Loading payment status…</p>}</Panel>;
}
