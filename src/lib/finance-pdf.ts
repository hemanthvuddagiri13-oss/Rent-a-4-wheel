...
export async function renderFinancePdf(snapshot:Record<string,unknown>){
 ...
 const walk=(value:unknown,key="",depth=0,currency=String(snapshot.currency))=>{
  if(value===undefined||value===null)return;
  if(Array.isArray(value)){text(label(key),true);if(!value.length)text("No entries in this snapshot.");for(let i=0;i<value.length;i++){text(`Entry ${i+1}`,true);walk(value[i],"",depth+1,currency);}return;}
  if(typeof value==="object"){if(key)text(label(key),true);for(const [k,v]of Object.entries(value))walk(v,k,depth+1,String((value as {currency?:string}).currency??currency));return;}
  const shown=typeof value==="number"&&key.endsWith("Cents")?`${(value/100).toFixed(2)} ${currency.toUpperCase()} (${value} minor units)`:String(value);
