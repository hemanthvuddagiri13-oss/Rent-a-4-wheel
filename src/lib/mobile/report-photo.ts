...
export async function readReportPhoto(userId: string, reservationId: string, reportId: string, photoId: string) {
  const object = await prisma.$transaction(async tx => {
    ...
    await tx.auditLog.create({ data: { actorId: userId, action: "mobile.report_photo.read", entityType: "ConditionPhoto", entityId: photoId } });
    return stored;
  });
