import type { Payment, Refund, Reservation, SecurityDeposit } from "@prisma/client";
...
type FinancialSnapshot = Pick<Reservation, "status" | "financialDisposition" | "depositCents"> & {
  payments: Payment[]; refunds: Refund[]; deposit: SecurityDeposit | null;
};
...
export function financialProjection(r: FinancialSnapshot, now = new Date()) {
  ...
  const pendingRefundCents = refunds.filter(f => f.status === "PENDING").reduce((n, f) => n + f.amountCents, 0);
  ...
  const d = r.deposit;
  const depositValid = r.depositCents === 0 || Boolean(d && d.status === "SUCCEEDED" && d.stripeStatus === "requires_capture" && d.amountCents >= r.depositCents && d.authorizationExpiresAt && d.authorizationExpiresAt > now && !d.releasedAt);
