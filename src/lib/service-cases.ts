...
export const caseCommandSchema = z.object({ action: z.enum(["reply", "internal", "transition", "assign", "escalate", "appeal", "satisfaction", "override", "safetyClear"]), version: z.coerce.number().int().min(0), body: z.string().min(1).max(5000), state: z.string().optional(), assigneeId: z.string().optional(), rating: z.coerce.number().int().min(1).max(5).optional(), stepUpCode: z.string().regex(/^\d{6}$/).optional(), confirm: z.enum(["yes", "no"]).optional() });
export async function caseCommand(userId: string, id: string, input: unknown, db: PrismaClient = prisma) {
  ...
  return db.$transaction(async tx => {
    ...
    await tx.serviceCaseEvent.create({ data: { caseId: id, actorId: userId, action: data.action.toUpperCase(), fromState: c.state, toState: state, body, internal: ["internal", "assign", "escalate"].includes(data.action), version: c.version + 1 } });
