...
export async function confirmAfterRentalPaymentSuccess(
  ...
  const eligible = await withReservationLock(reservation.id, async tx => {
    ...
    try{await requireReservationJurisdiction(tx,current.id,"CONFIRMATION");}catch{await requireRefund(tx,current.id,payment,"Jurisdiction disabled before confirmation");return false;}
