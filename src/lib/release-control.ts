...
export async function requireReleaseFeature(key:ReleaseFeatureKey,tx:Prisma.TransactionClient=prisma){
 ...
 const approved=await tx.policyApproval.findMany({where:{kind:{in:[...policies[key]]},jurisdiction:"US-TX",status:"APPROVED",effectiveAt:{lte:new Date()},approvedAt:{not:null},approvedById:{not:null}},select:{kind:true,contentHash:true,professionalReviewRequired:true,professionalReference:true}});
...
export async function setReleaseFeature(userId:string,key:ReleaseFeatureKey,enabled:boolean,code:string,reason:string){
 ...
 return prisma.$transaction(async tx=>{
  ...
  if(enabled)await requireReleaseFeature(key,tx);
...
export async function registerPolicy(userId:string,input:{kind:string;version:string;contentHash:string;professionalReference:string;effectiveAt:string;code:string;supersedesId?:string}){
 if(!(POLICY_KINDS as readonly string[]).includes(input.kind)||!/^\d{1,4}(\.\d{1,4}){0,3}$/.test(input.version)||!/^[a-f0-9]{64}$/.test(input.contentHash)||input.professionalReference.length<10||input.professionalReference.length>500||!Number.isFinite(Date.parse(input.effectiveAt)))throw new Error("INVALID_POLICY_APPROVAL");
 await securityStepUp(userId,input.code);
 return prisma.$transaction(async tx=>{
  await tx.$queryRaw`SELECT pg_advisory_xact_lock(hashtextextended('release-control',0))::text`;
  const actor=await tx.user.findUnique({where:{id:userId}});if(!actor?.isActive||actor.role!=="SUPER_ADMIN")throw new ReleaseGateError();
  if(input.supersedesId){const old=await tx.policyApproval.findUniqueOrThrow({where:{id:input.supersedesId}});if(old.kind!==input.kind||old.jurisdiction!=="US-TX")throw new Error("INVALID_POLICY_APPROVAL");}
  const row=await tx.policyApproval.create({data:{kind:input.kind,version:input.version,contentHash:input.contentHash,jurisdiction:"US-TX",professionalReviewRequired:true,professionalReference:input.professionalReference,approvedById:userId,approvedAt:new Date(),effectiveAt:new Date(input.effectiveAt),status:"APPROVED",supersedesId:input.supersedesId}});
