...
export async function storePrivateDocument(buffer:Buffer,mimeType:string,stableId=randomUUID()):Promise<StoredDocument>{
 ...
 const needsWrite=await db.$transaction(async tx=>{
 ...
 await db.privateObject.update({where:{key},data:{writeState:"STORED"}});
 }catch{await db.privateObject.updateMany({where:{key,writeState:"RUNNING"},data:{writeState:"UNCERTAIN"}});throw new Error("PRIVATE_WRITE_REQUIRES_REVIEW");}
...
export async function deletePrivateBytes(key:string){
 return withPrivateStorageGuard(key,async db=>{
 const row=await db.privateObject.findUnique({where:{key}});
 if(row&&(!["DELETING","DELETED"].includes(row.state)||row.writeState!=="STORED"))throw new Error("PRIVATE_DELETE_REQUIRES_REVIEW");
