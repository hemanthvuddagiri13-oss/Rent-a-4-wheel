...
function client(){
 if(!process.env.PRIVATE_STORAGE_ENDPOINT?.startsWith("https://")&&!localDevelopment())throw new Error("PRIVATE_STORAGE_TLS_REQUIRED");
 return new S3Client({region:process.env.PRIVATE_STORAGE_REGION,endpoint:process.env.PRIVATE_STORAGE_ENDPOINT,forcePathStyle:true,maxAttempts:1,requestHandler:{connectionTimeout:3000,requestTimeout:10000,throwOnRequestTimeout:true},credentials:{accessKeyId:process.env.PRIVATE_STORAGE_ACCESS_KEY_ID!,secretAccessKey:process.env.PRIVATE_STORAGE_SECRET_ACCESS_KEY!}});
}
...
export async function verifyPrivateBucket(){
 ...
 }catch{throw new Error("PRIVATE_BUCKET_UNAVAILABLE_OR_UNSAFE");}finally{s3.destroy();}
}
export async function readS3Private(key:string){const s3=client();try{const result=await s3.send(new GetObjectCommand({Bucket:bucket(),Key:objectKey(key),ChecksumMode:"ENABLED"}));if(!result.Body||result.ServerSideEncryption!=="aws:kms"||result.SSEKMSKeyId!==process.env.PRIVATE_STORAGE_KMS_KEY_ID)throw new Error("PRIVATE_OBJECT_INVALID");if(!result.ContentLength||result.ContentLength>16*1024*1024)throw new Error("PRIVATE_OBJECT_SIZE");const chunks:Uint8Array[]=[];let size=0;for await(const chunk of result.Body as AsyncIterable<Uint8Array>){size+=chunk.length;if(size>16*1024*1024)throw new Error("PRIVATE_OBJECT_SIZE");chunks.push(chunk);}if(size!==result.ContentLength)throw new Error("PRIVATE_OBJECT_SIZE");return Buffer.concat(chunks);}catch{throw new Error("PRIVATE_READ_FAILED");}finally{s3.destroy();}}
...
export async function deleteS3Private(key:string){
 const s3=client(),Key=objectKey(key);try{
  ...
  for(let page=0;page<10;page++){const list=await s3.send(new ListObjectVersionsCommand({Bucket:bucket(),Prefix:Key,MaxKeys:100}));const versions=[...(list.Versions??[]),...(list.DeleteMarkers??[])].filter(v=>v.Key===Key);if(!versions.length){if(list.IsTruncated)throw new Error("PRIVATE_DELETE_REMAINS_PENDING");return;}for(const version of versions){if(!version.VersionId)throw new Error("PRIVATE_DELETE_VERSION_UNKNOWN");await s3.send(new DeleteObjectCommand({Bucket:bucket(),Key,VersionId:version.VersionId}));}}
  throw new Error("PRIVATE_DELETE_REMAINS_PENDING");
 }catch{throw new Error("PRIVATE_DELETE_REMAINS_PENDING");}finally{s3.destroy();}
}
