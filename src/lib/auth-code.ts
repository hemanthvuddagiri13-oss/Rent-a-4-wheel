 ...
 */
export async function verifyAuthCode(params: { email: string; code: string; ip: string | null; purpose?: "MOBILE_SIGN_IN" | "SIGN_IN" | "EMERGENCY_OVERRIDE_STEP_UP" | "FINANCE_STEP_UP" | "SECURITY_STEP_UP" }): Promise<VerifyCodeResult> {
  const email = normalizeEmail(params.email);
