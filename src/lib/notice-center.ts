...
export async function projectTransactionalNotices() {
  ...
  for (const n of notices) {await prisma.$transaction(tx=>enqueueNoticeEmail(tx,n.userId,n.category,n.eventKey,n.required));committed++;}
  ...
  for (const r of upcoming) { const eventKey = (r.status==="ACTIVE"?"return-reminder:":"pickup-reminder:")+r.id+":"+(r.status==="ACTIVE"?r.returnAt:r.pickupAt).toISOString();await prisma.$queryRaw`SELECT community_reservation_notice(${r.id},${eventKey},${r.status==="ACTIVE"?"Your trip return is approaching":"Your pickup is approaching"},'TRIP')::text`; committed++; }
  return {...workerResult({committed,attempted:committed}),projected:rows.length,emailsPlanned:notices.length,reminders:upcoming.length};
  }catch{return workerResult({committed,attempted:committed+1,failed:1});}
}
