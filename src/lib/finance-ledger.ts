...
/** A normal return is not evidence of completed accounting. REVIEW takes
 * precedence even when some immutable postings were retained in this pass.
 * NO_WORK means the complete economic checkpoint and journals did not change.
 * Exceptions remain failures and roll back the caller's transaction. */
export type AccountingOutcome={status:"COMMITTED"|"REVIEW"|"NO_WORK"};
// Caller holds the approved reservation lock. No provider calls occur here.
export async function accountReservation(tx:Prisma.TransactionClient,id:string):Promise<AccountingOutcome>{
 const before=await tx.accountingCheckpoint.findUnique({where:{reservationId:id}});
 ...
 if(!(await ensureRefundCompatibility(tx,id)).valid){await certifyAccounting(tx,id);return {status:"REVIEW"};}
 ...
 const evidence=await certifyAccounting(tx,id);
 // Certification reasons and unresolved holds are not completed accounting,
 // including refund suspense/unmatched refunds and compatibility review.
 if(evidence.reasons.length||await tx.financeIssue.count({where:{reservationId:id,status:{not:"RESOLVED"}}}))return {status:"REVIEW"};
