...
async function renderFinancialPdf(title:string,lines:string[]){const doc=await PDFDocument.create(),font=await doc.embedFont(StandardFonts.Helvetica);doc.setCreationDate(new Date(0));doc.setModificationDate(new Date(0));let page=doc.addPage([612,792]),y=748;const draw=(text:string,bold=false)=>{for(const piece of text.replace(/[^\x20-\x7e]/g," ").match(/.{1,90}(?:\s|$)|.{1,90}/g)??[""]){if(y<50){page=doc.addPage([612,792]);y=748;}page.drawText(piece.trim(),{x:40,y,size:bold?16:10,font,color:bold?rgb(.55,.4,.1):rgb(.12,.12,.12)});y-=bold?26:16;}};draw("Rent A 4Wheel",true);draw(title.replaceAll("_"," "),true);for(const line of lines)draw(line);return Buffer.from(await doc.save());}
export async function issueFinanceDocument(userId:string,input:{kind:string;reservationId?:string;batchId?:string;period?:string;newVersion?:boolean}){
 ...
 return prisma.$transaction(async tx=>{
  ...
  const version=(previous?.version??0)+1;Object.assign(snapshot,{version,snapshotHash:fingerprint(snapshot)});const lines=JSON.stringify(snapshot,null,2).split("\n"),pdf=await renderFinancialPdf(input.kind,lines),contentHash=createHash("sha256").update(pdf).digest("hex");
