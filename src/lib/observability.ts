...
export function failedWorkerBatch(value:unknown){const r=value as {failed?:number;completed?:number;delivered?:number;stale?:number;review?:number;processed?:number};return Boolean((r?.failed??0)+(r?.stale??0)+(r?.review??0))&&!(r.completed??r.delivered??Math.max(0,(r.processed??0)-(r.failed??0)));}
export async function observeWorker<T>(name:string,run:()=>Promise<T>):Promise<T>{const start=Date.now();try{const result=await run();const failed=failedWorkerBatch(result);await recordOperationalEvent(failed?"CRON_FAILED":"CRON_COMPLETE",failed?"CRITICAL":"INFO",undefined,1,name,Math.max(1,Date.now()-start));return result;}catch(error){safeLog("CRON_FAILED",error);await recordOperationalEvent("CRON_FAILED","CRITICAL",undefined,1,name,Math.max(1,Date.now()-start));throw new Error("WORKER_RETRY_REQUIRED");}}
