...
import type {OperationsJob} from "@prisma/client";
import {assertOperationsLease,finishOperation} from "@/lib/operations-lease";
 ...
 */
export async function generateSignedAgreementArtifact(acceptanceId:string,job?:OperationsJob):Promise<boolean>{
 const artifact=await prisma.$transaction(async tx=>{
  await tx.$queryRaw`SELECT financial_guard_xact(${"agreement-artifact:"+acceptanceId})`;
  if(job)await assertOperationsLease(tx,job);
 ...
 if(!artifact)return job?Boolean((await finishOperation(job,"DONE")).count):true;
 ...
 return prisma.$transaction(async tx=>{
  await tx.$queryRaw`SELECT financial_guard_xact(${"agreement-artifact:"+acceptanceId})`;
  if(job&&!(await finishOperation(job,"DONE",undefined,tx)).count)return false;
