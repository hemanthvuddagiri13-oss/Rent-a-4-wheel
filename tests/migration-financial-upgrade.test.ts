...
describe('populated financial schema upgrades',()=>{
 it.each(['20260916210000_add_deposit_recovery_window_expired_reason','20260916220000_durable_financial_operations'])('quarantines unknown results and preserves known provider IDs upgrading %s',async cutoff=>{
  ...
  try {
   ...
   sql(db,['-c',`UPDATE "Refund" SET "stripeRefundId"=NULL WHERE "id"='mig_test_refund';
   ...
   expect(refund.legacyUncertain).toBe(true);expect(refund.idempotencyKey).toBe('legacy-mig_test_refund');expect(refund.status).toBe('PENDING');
   ...
   expect(await client.financialOperation.count()).toBe(0);
