...
it.skipIf(!enabled)("records the approved real application baseline by route, role and viewport without asserting visual acceptance",async()=>{
 ...
 const payout=await prisma.payoutBatch.create({data:{hostId:host.hostProfile.id,accountId:"acct_synthetic_baseline",amountCents:0,currency:"usd",reason:"Synthetic view only. Live payouts are disabled."}});
 ...
 try{
  ...
  for(const spec of specs){
   const page=await contexts[spec.role].newPage();const response=await page.goto(base+spec.route,{waitUntil:"networkidle",timeout:90000});await page.evaluate(()=>document.fonts.ready);
   ...
   for(const [width,height] of sizes){
    ...
    const metrics=await page.evaluate(()=>{
     ...
     return {title:document.title,h1:[...document.querySelectorAll("h1")].map(e=>e.textContent),overflow:document.documentElement.scrollWidth>innerWidth,smallTargets:controls.filter(e=>{const r=e.getBoundingClientRect();return r.width<44||r.height<44;}).length,unlabelledFields:[...document.querySelectorAll<HTMLInputElement>("input:not([type=hidden]),select,textarea")].filter(visible).filter(e=>!e.labels?.length&&!e.getAttribute("aria-label")&&!e.getAttribute("aria-labelledby")).length,mainLandmarks:document.querySelectorAll("main").length};
    });
    const image=spec.name+"-"+width+".png";await page.screenshot({path:output+"/"+image,fullPage:true,animations:"disabled",mask:[sensitive]});captures.push({...spec,width,height,status:response?.status(),image,...metrics});
   }
   await page.close();
  }
