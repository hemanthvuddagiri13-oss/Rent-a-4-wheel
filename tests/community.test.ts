...
it("case overrides require an isolated one-use step-up code and never clear financial review",async()=>{
 ...
 await caseCommand(superAdmin.id,c.id,{action:"assign",assigneeId:superAdmin.id,version:0,body:"Independent agent assignment"});
 const input={action:"override",version:1,body:"Exceptional decision after reviewing all available evidence",confirm:"yes",stepUpCode:"123456"};
 await prisma.authCode.create({data:{email:superAdmin.email,codeHash:await bcrypt.hash("123456",4),purpose:"SIGN_IN",expiresAt:new Date(Date.now()+60000)}});await expect(caseCommand(superAdmin.id,c.id,input)).rejects.toThrow("step-up");await expect(caseCommand(f.agent.id,c.id,input)).rejects.toThrow();
 await prisma.authCode.create({data:{email:superAdmin.email,codeHash:await bcrypt.hash("123456",4),purpose:"EMERGENCY_OVERRIDE_STEP_UP",expiresAt:new Date(Date.now()+60000)}});await caseCommand(superAdmin.id,c.id,input);expect(await prisma.serviceCase.findUnique({where:{id:c.id}})).toMatchObject({state:"RESOLVED"});expect(await prisma.reservation.findUnique({where:{id:f.r.id}})).toMatchObject({financialDisposition:"REVIEW"});await expect(caseCommand(superAdmin.id,c.id,{...input,version:2})).rejects.toThrow();expect(await prisma.auditLog.count({where:{actorId:superAdmin.id,action:"case.step_up_override"}})).toBe(1);
});
