...
describe("Batch 1C financial regressions",()=>{
 ...
 it("two overlapping renewals cannot release a replacement that subsequently starts a trip", async () => {
   ...
   const second=attemptDepositAuthorization(stale,rental,true);
   release.release();await Promise.all([first,second]);
   await withReservationLock(r.id,tx=>assertFinancialTripStart(tx,r.id),a);
 ...
 it("serializes refund takeover with real projections and an immutable provider ledger",async()=>{
   const {r,p}=await fixture();const refund=await getOrCreateRefundOperation({reservationId:r.id,paymentId:p.id,amountCents:15000,idempotencyKey:`takeover:${r.id}`});
   const entered=barrier(),release=barrier();const ledger=new Map<string,{hash:string,result:{id:string,status:string}}>();
   provider.refunds.create.mockImplementation(async(params,options)=>{const hash=fingerprint(params);ledger.set(options.idempotencyKey,{hash,result:{id:`re_takeover_${r.id}`,status:"succeeded"}});entered.release();await release.wait;return ledger.get(options.idempotencyKey)!.result;});
   const first=executeRefundOperation(refund.id,p.stripePaymentIntentId);await entered.wait;
   const [{pid}]=await b.$queryRaw<Array<{pid:number}>>`SELECT pg_backend_pid() AS pid`;
   const takeover=withReservationLock(r.id,async tx=>{const op=await tx.financialOperation.findUniqueOrThrow({where:{key:refund.idempotencyKey}});expect(op.state).toBe("OBSERVED");expect(op.leaseToken).toBeNull();},b);
   try{await waitLock(pid)}finally{release.release()}await Promise.all([first,takeover]);
   await executeRefundOperation(refund.id,p.stripePaymentIntentId);
   expect(ledger.size).toBe(1);expect(provider.refunds.create).toHaveBeenCalledTimes(1);expect((await prisma.refund.findUniqueOrThrow({where:{id:refund.id}})).status).toBe("SUCCEEDED");expect(await prisma.outboxMessage.count({where:{deliveryKey:`refund:${refund.id}`}})).toBe(1);
 });
 it("advances eligible refunds beyond a 25-item uncertain backlog using the worker's actual client",async()=>{const {r,p}=await fixture();const ids:string[]=[];for(let i=0;i<30;i++){const f=await getOrCreateRefundOperation({reservationId:r.id,paymentId:p.id,amountCents:100,idempotencyKey:`backlog:${r.id}:${i}`});ids.push(f.id);await prisma.refund.update({where:{id:f.id},data:{updatedAt:new Date(i)}})}provider.refunds.create.mockImplementation(async params=>{if(ids.indexOf(params.metadata.refundId)<25)throw new Error("Uncertain outcome");return{id:`re_${params.metadata.refundId}`,status:"succeeded"}});const original=workerDb.refund.findMany.bind(workerDb.refund);const spy=vi.spyOn(workerDb.refund,"findMany").mockImplementation(args=>original({...args,where:{...args?.where,id:{in:ids}}}) as never);await recoverRefunds();await recoverRefunds();expect(spy).toHaveBeenCalledTimes(4);expect(await prisma.refund.count({where:{id:{in:ids.slice(25)},status:"SUCCEEDED"}})).toBe(5);expect(await prisma.refund.count({where:{id:{in:ids.slice(0,25)},status:"PENDING"}})).toBe(25);});
});
