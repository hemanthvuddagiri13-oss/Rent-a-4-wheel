...
const additions = ["20260923000000_mobile_credentials", "20260923001000_mobile_upload_intent", "20261001010000_mobile_phone_identity"];
...
it("upgrades populated web auth and frozen financial evidence without granting native access or releasing holds", async () => {
  const user = await db.user.create({ data: { email: "synthetic-upgrade@mobile.test", phone: "+12025550101", customer: { create: {} } } });
  ...
  expect(await db.mobilePhoneIdentity.count()).toBe(0); expect(await db.mobileLoginChallenge.count()).toBe(0);
  expect(await db.mobileVerificationReceipt.count()).toBe(0); expect(await db.mobileIdentityRecovery.count()).toBe(0);
