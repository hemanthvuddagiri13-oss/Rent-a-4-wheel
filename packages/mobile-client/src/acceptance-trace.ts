...
function trace(phase: string, target: string | undefined, requestId?: string, status?: number) {
  ...
  try { void deliver('http://localhost:3000/__native-ui-trace', { method: 'POST', credentials: 'omit', headers: { 'content-type': 'application/json' }, body: JSON.stringify(row) }).catch(() => {}); } catch { /* Diagnostics cannot interrupt a control handler. */ }
}
