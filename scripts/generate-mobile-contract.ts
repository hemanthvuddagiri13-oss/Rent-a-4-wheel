...
const error = { type: "object", additionalProperties: false, required: ["data", "error", "requestId"], properties: { data: { type: "null" }, error: { type: "object", additionalProperties: false, required: ["code"], properties: { nonCommit: { type: "object", additionalProperties: false, required: ["idempotencyKey"], properties: { idempotencyKey: { type: "string", pattern: "^[A-Za-z0-9_-]{16,128}$" } }, description: "Only message.send, case.reply and case.open: durable terminal rejection receipt after rollback, bound to the exact request key. Status alone never proves non-commit." }, code: { type: "string", enum: ["UNAUTHORIZED", "FORBIDDEN", "INVALID_REQUEST", "NOT_FOUND", "CONFLICT", "RATE_LIMITED", "UNAVAILABLE"] } } }, requestId: { type: "string", format: "uuid" } } };
