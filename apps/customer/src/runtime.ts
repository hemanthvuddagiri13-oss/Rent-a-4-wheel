...
export const vault = { get: () => SecureStore.getItemAsync('ra4w.session', options), set: (v: string) => SecureStore.setItemAsync('ra4w.session', v, options), clear: () => SecureStore.deleteItemAsync('ra4w.session', options) };
...
export async function deviceId() {
  const saved = await SecureStore.getItemAsync('ra4w.device', options); if (saved) return saved;
  const id = Crypto.randomUUID(); await SecureStore.setItemAsync('ra4w.device', id, options); return id;
}
