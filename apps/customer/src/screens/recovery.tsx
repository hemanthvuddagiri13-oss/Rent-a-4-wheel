...
export function Recovery() {
  useEffect(() => { trace('mount', 'Recovery'); return () => trace('unmount', 'Recovery'); }, []);
  ...
  useEffect(() => { trace(q.isPending || !protectedScreen ? 'loading' : q.isError ? 'error' : 'loaded', 'Recovery'); }, [q.isPending, q.isError, protectedScreen]);
  if (!protectedScreen) return <Page title="Private screen"><Hint>Preparing screen protection.</Hint></Page>;
  return <Page title="Interrupted requests"><Hint>Private request text and identity details are not stored in this list. Recovery asks the server whether the original request committed. If it did not, the server closes that request so you can safely enter it again. Nothing is resubmitted automatically.</Hint><Button title="Refresh interrupted requests" onPress={() => void q.refetch()} />{q.isPending ? <Busy /> : q.isError ? <ErrorText message={friendly(q.error)} /> : !q.data.length ? <Copy>No interrupted requests for this account.</Copy> : q.data.map(record => <Card key={record.key}><Copy>{labels[record.operation] ?? 'Request'}</Copy><Hint>Started {new Date(record.createdAt).toLocaleString()}</Hint><Hint>Open the original conversation or trip to review its current state before recovering.</Hint><Button testID={'recover-' + record.operation} title={`Recover ${labels[record.operation] ?? 'request'}`} disabled={action.busy || record.operation === 'privateUpload'} onPress={() => void action.run(async () => { trace('action', 'recover-' + record.operation); const result = await recoverRequest(record); setConfirmed(result.outcome === 'COMMITTED' ? 'Server acknowledged ' + record.operation + '. Open the original screen to verify its current state.' : 'Server confirmed this request did not commit and has closed its original key. Return to the original screen to enter your request again.'); await queries.invalidateQueries(); })} /></Card>)}<ErrorText message={action.error} /><Hint>{confirmed}</Hint><Hint>For an interrupted photo upload, return to its original screen and select the same photo. Private image bytes are not saved in this recovery list.</Hint></Page>;
}
