...
export function Inspection({ route }: NativeStackScreenProps<Routes, 'Inspection'>) {
  ...
  return <Page title="Vehicle condition"><Hint>Record the vehicle before departure and at return. Include exterior, interior and any damage. Each party performs and accepts their own report. Do not include people, licenses or payment information in condition photos.</Hint>
    <Button title={phase === 'PRE_TRIP' ? 'Pre-trip selected â€” switch to return' : 'Return selected â€” switch to pre-trip'} onPress={() => setPhase(p => p === 'PRE_TRIP' ? 'POST_TRIP' : 'PRE_TRIP')} />
    <Field label="Odometer mileage" value={mileage} onChangeText={setMileage} keyboardType="number-pad" /><Field label="Fuel level (0â€“100 percent)" value={fuel} onChangeText={setFuel} keyboardType="number-pad" /><Field label="Damage notes" value={notes} onChangeText={setNotes} multiline maxLength={2000} />
    ...
    <Button title="Refresh saved reports" onPress={() => { void q.refetch(); }} />{q.isPending ? <Busy /> : q.isError ? <ErrorText message={friendly(q.error)} /> : q.data.items.map(r => <Card key={r.id}><Copy>{r.phase} Â· {r.submittedByRole}</Copy><Hint>{r.mileage} miles Â· {r.fuelLevel}% fuel</Hint><Copy>{r.damageNotes || 'No damage notes'}</Copy>{r.photos.map(p => <PrivateEvidence key={p.id} report={{ id, reportId: r.id, photoId: p.id }} label={p.category + ' photo'} />)}{r.acceptedAt ? <Hint>Accepted {new Date(r.acceptedAt).toLocaleString()}</Hint> : r.own ? <Button title="Accept my report as accurate" disabled={action.busy} onPress={() => void action.run(async () => { await mutate('acceptReport', { params: { id, reportId: r.id }, body: {} }); await q.refetch(); })} /> : <Hint>Only the author of this report can accept it.</Hint>}</Card>)}</Page>;
}
