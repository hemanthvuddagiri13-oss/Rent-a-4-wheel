...
At 534e24b, host iOS recovered the interrupted message and support reply, then the authoritative `hostTrip` read returned HTTP 500 (request `a19ba575-8a0f-4cee-982a-22b217f555bb`, 17:02:43 UTC). The screenshot displays that request reference and the fail-closed unavailable state; this is not a driver failure. Existing telemetry did not preserve an exception category, so its underlying cause is not established. Mobile HTTP telemetry now includes only a fixed failure category for server errors, with regression coverage proving exception text, metadata and request credentials are absent. No transaction timeout, retry, authority or native assertion was changed to mask that failure.

Visual inspection at 092525e found one Android price capture with partially painted text/controls, while later reservation captures render normally. The price capture now centers the actual breakdown, waits for animation settlement and requires its vehicle-price row before capture. This is a screenshot barrier, not a retry or replacement of booking assertions.
