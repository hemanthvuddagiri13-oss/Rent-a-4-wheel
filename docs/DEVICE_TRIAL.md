...
3. Build separately for each mode. Set `APP_ENV=staging`, `NATIVE_ACCEPTANCE=0`, `EXPO_PUBLIC_APP_MODE=customer` or `host`, `EXPO_PUBLIC_API_ORIGIN` to the verified HTTPS staging origin, `ALLOW_DEV_PAYMENT_SIMULATION=false`, `LIVE_FINANCE_ENABLED=false` and a non-fixture `MOBILE_SMS_PROVIDER`. Use `npm --prefix apps/customer run device-trial:android-prebuild` or `device-trial:ios-prebuild`, which invokes the configuration guard before native prebuild. This local check does **not** validate server/provider deployment, and it does not produce an installable signed binary.
