...
Base: approved main `e5e2564a5a5f753e8db2cc2408efd843e0cb1fbf`. Branch: `codex/phase-7b-customer-app`. This is a draft implementation for independent review, not production approval. The phone-first requirement adds migration `20261001010000_mobile_phone_identity`; approved migrations are unchanged. Live payments and payouts remain disabled.
...
Native acceptance uses explicit SMS provider fixtures and a synthetic verified phone/email identity. It exercises the real challenge, single-use consumption, bearer authentication and logout routes; it does not prove SMS or email delivery. PostgreSQL/HTTP tests replace only provider boundaries and exercise registration, concurrent verification, rate limits, linking, phone changes and blocked recovery. Unit tests cover serialized refresh, concurrent 401s, lost refresh, crash marker, revocation, storage failure, offline mutation recovery and logout failure. Existing PostgreSQL mobile tests retain upload failures/retries, revoked access, booking races and trip gates. Camera hardware, external SMS/email/scanner/storage, VoiceOver/TalkBack and production-network behavior require device/staging acceptance; do not infer them from automated unit tests.

The installed-app email journey is **code-verification coverage only**, not new email-code issuance: five pre-seeded AuthCode records trigger the request endpoint's generic rate-limit response, and the app consumes the one unconsumed fixture code. Newly issued email codes are captured and consumed in the isolated PostgreSQL/HTTP delivery-sink tests, not the installed-app journey. Neither fixture proves external delivery.
