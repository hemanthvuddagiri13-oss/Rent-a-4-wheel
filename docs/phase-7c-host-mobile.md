...
Private reads snapshot the resolved legacy chain and immutable manifest identity in one database snapshot, verify the downloaded bytes, then reload and validate the chain after provider I/O. The mobile identity response repeats this validation after its final session/document authorization. A validated legacy source may remain quarantined; the effective target must remain CLEAN/STORED, undeleted and identical. Six real HTTP/PostgreSQL barrier cases cover direct/legacy quarantine, deletion, uncertain storage state, changed content identity and remapped legacy targets while document metadata remains unchanged. Only external provider-byte delivery is paused; storage validation and HTTP authorization execute for real.
