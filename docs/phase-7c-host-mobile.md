...
## Focused review correction: effective object and incident acceptance

Private reads snapshot the resolved legacy chain and immutable manifest identity in one database snapshot, verify the downloaded bytes, then reload and validate the chain after provider I/O. The mobile identity response repeats this validation after its final session/document authorization. A validated legacy source may remain quarantined; the effective target must remain CLEAN/STORED, undeleted and identical. Six real HTTP/PostgreSQL barrier cases cover direct/legacy quarantine, deletion, uncertain storage state, rejected immutable-identity and legacy-mapping rewrites followed by quarantine while document metadata remains unchanged. Only external provider-byte delivery is paused; storage validation and HTTP authorization execute for real.

Host incident acceptance now requires the authoritative loaded case title (a dedicated heading selector) and loaded events. Both installed platforms deliberately reject submission once and reject detail readback once. Each negative run must fail at that exact loaded-detail assertion and include matching current proxy fault telemetry; driver failures or unrelated assertions fail CI. PostgreSQL then proves zero incidents after rejected submission and exactly one correctly scoped incident after committed submission. After readback recovery, the same loaded-detail assertions and a fresh count/scope/opening-event check precede the screenshot. Synthetic fault controls exist only in the disposable CI proxy. Four harness tests reject missing evidence, wrong failures and unexpectedly successful acceptance.

No schema, migration, financial authority, recovery ownership or live-provider configuration changes are part of this correction. Exact-SHA CI and installed-app screenshots, rather than these planned assertions alone, establish its verification results.
