...
Sign-in challenges freeze an existing phone owner and identity version. Finalization locks sorted phone targets, then User, then the challenge; it re-reads the exact binding and version under the User lock before session issuance. Phone change locks both old and replacement targets, changes the binding and revokes sessions atomically. Linking/changing invalidates outstanding sign-in/registration challenges for the affected numbers. An old challenge cannot become registration or authenticate a recycled binding. Concurrent first registrations remain serialized by target and converge on one account; authenticated linking invalidates earlier unbound registration approvals.

Regression tests use independent PostgreSQL connections and explicit transaction barriers for both commit orders. They release the winning transaction only after `pg_blocking_pids` confirms contention, then verify old access/refresh rejection, new ownership, exact session counts and replay rejection. No timed sleeps establish race ordering. Additional tests cover stale versions, removed bindings and an approved registration racing authenticated linking. No schema change is required.
