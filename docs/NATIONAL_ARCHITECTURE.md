...
These tests do not prove state legal compliance, live-provider behavior, real insurance/tax approval or production readiness. Acceptance requires the complete migration, route, browser, security and repeated-concurrency verification matrix at the exact PR HEAD; the final report records that commit and CI evidence. Passing those checks does not enable a production jurisdiction or promote sample pricing.
